...
class LSTM(RNNMixin, BackendHandler):
  ...
  def _common(cls, node, **kwargs):
    ...
    batch_size = 1
    state_c = tf.compat.v1.placeholder(tf.float32, [batch_size, hidden_size], name=state_c_name)
    state_h = tf.compat.v1.placeholder(tf.float32, [batch_size, hidden_size], name=state_h_name)
    ...
    with tf.compat.v1.variable_scope(
      ...
      rnn_kwargs["state_c"] = state_c
      rnn_kwargs["state_h"] = state_h
    ...
    if num_directions == 1:
      state = states[0]
      state_c_out_name, state_h_out_name = RNNState.gen_lstm_output_state_names(False)
      c = tf.expand_dims(state[0], 0, name=state_c_out_name)
      h = tf.expand_dims(state[1], 0, name=state_h_out_name)
